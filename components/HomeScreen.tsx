    ...
    <div className="relative flex h-full min-h-screen w-full flex-col max-w-3xl mx-auto bg-background-light dark:bg-background-dark pb-32 transition-colors duration-300">
      ...
      <header className="flex items-center justify-between px-6 pt-6 pb-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm text-neutral-500 dark:text-neutral-400 font-medium">{t('today')}</p>
            <h2 className="text-xl font-bold leading-tight dark:text-white">
                {user ? `${t('hello')}, ${user.name.split(' ')[0]}` : t('welcome')}
            </h2>
          </div>
        </div>
      ...
      <main className="flex flex-col px-6 animate-enter">
        ...
        <div className="flex flex-col items-center justify-center py-8">
          <div className="relative flex items-center justify-center size-64">
            ...
            <div className="absolute inset-4 rounded-full bg-background-light dark:bg-background-dark flex flex-col items-center justify-center z-10">
              ...
              <p className="text-sm font-medium text-neutral-400 uppercase tracking-wider mt-1">{t('kcal_left')}</p>
              
              <div className="mt-4 flex items-center gap-2 bg-surface-light dark:bg-surface-dark px-3 py-1.5 rounded-full">
                <div className="size-2 rounded-full bg-primary"></div>
                <span className="text-xs font-semibold text-neutral-600 dark:text-neutral-300">{t('target')}: {targets.calories}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-center gap-8 mt-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-neutral-600 dark:text-neutral-400">{t('consumed')}</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-[#F8DDA4]"></div>
              <span className="text-sm font-medium text-neutral-600 dark:text-neutral-400">{t('remaining')}</span>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="flex flex-col bg-surface-light dark:bg-surface-dark p-4 rounded-2xl shadow-sm border border-transparent dark:border-neutral-800 hover:border-primary/20 transition-all">
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-semibold text-neutral-700 dark:text-neutral-200">{t('protein')}</span>
              ...
              <p className="text-xs text-neutral-500 font-medium">{proteinLeft}g {t('left')}</p>
            </div>
            
            {/* Carbs */}
            <div className="flex flex-col bg-surface-light dark:bg-surface-dark p-4 rounded-2xl shadow-sm border border-transparent dark:border-neutral-800 hover:border-primary/20 transition-all">
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-semibold text-neutral-700 dark:text-neutral-200">{t('carbs')}</span>
              ...
              <p className="text-xs text-neutral-500 font-medium">{carbsLeft}g {t('left')}</p>
            </div>
            
            {/* Fat */}
            <div className="flex flex-col bg-surface-light dark:bg-surface-dark p-4 rounded-2xl shadow-sm border border-transparent dark:border-neutral-800 hover:border-primary/20 transition-all">
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-semibold text-neutral-700 dark:text-neutral-200">{t('fat')}</span>
              ...
              <p className="text-xs text-neutral-500 font-medium">{fatLeft}g {t('left')}</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold dark:text-white">{t('recent_meals')}</h3>
            <button onClick={() => onNavigate('DIARY')} className="text-primary text-sm font-semibold hover:underline">{t('view_all')}</button>
          </div>
          <div className="flex flex-col gap-4">
            {recentMeals.length > 0 ? (
              ...
              <div className="flex items-center justify-center p-8 border-2 border-dashed border-neutral-200 dark:border-neutral-700 rounded-xl">
                 <p className="text-sm text-neutral-400 font-medium">{t('no_meals_today')}</p>
              </div>
            )}
            ...
            {caloriesLeft > 0 && (
              <div className="flex items-center justify-center p-6 border-2 border-dashed border-neutral-200 dark:border-neutral-700 rounded-xl cursor-pointer hover:bg-neutral-50 dark:hover:bg-white/5 transition-colors" onClick={() => onNavigate('ADD_MENU')}>
                <p className="text-sm text-neutral-400 font-medium">{t('log_next_meal')}</p>
              </div>
            )}
          </div>
        </div>
