...
const InsightsScreen: React.FC<InsightsScreenProps> = ({ onNavigate, onFabClick, user }) => {
  const { meals } = useData();
  const { t } = useLanguage();
  // 0. Fetch Weight History
  const [weightHistory, setWeightHistory] = React.useState<{ date: string | Date; weight: number }[]>([]);

  React.useEffect(() => {
    if (!user?.id) return;
    fetch(`/api/users/${user.id}/weight-history`)
      .then(res => res.ok ? res.json() : [])
      .then(data => setWeightHistory(data))
      .catch(err => console.error("Failed to fetch weight history", err));
  }, [user?.id, user?.weight]); // Refetch if weight changes
    ...
    <div className="relative flex h-full min-h-screen w-full flex-col max-w-md mx-auto bg-background-light dark:bg-background-dark pb-32 transition-colors duration-300">
      ...
      <main className="flex flex-col px-6 gap-8 animate-enter">
        ...
        <section className="mb-6">
          ...
          <div className="bg-white dark:bg-surface-dark rounded-3xl p-6 shadow-card border border-neutral-100 dark:border-neutral-800">
            {user ? (
              <div className="flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-3xl font-extrabold text-neutral-900 dark:text-white">
                      {user.weight ? user.weight : '--'} <span className="text-sm font-medium text-neutral-500">kg</span>
                    </div>
                    <span className="text-xs text-neutral-500">{t('current_weight')}</span>
                  </div>
                </div>

                <div className="h-40 w-full">
                  <WeightTrendChart history={weightHistory} currentWeight={Number(user.weight)} />
                </div>
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center text-center opacity-50 h-32">
